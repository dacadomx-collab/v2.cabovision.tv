...
$stmt = $pdo->prepare(
    "SELECT `articles`.`id`, `articles`.`title`, `articles`.`alias`, `articles`.`extract`,
     ...
     WHERE `articles`.`status_id` = :status_id AND `categories`.`alias` = :category_alias
...
<div class="arf-layout">
    <div class="arf-layout__main">
        ...
        <?php if (empty($articles)): ?>
            <p>No hay noticias publicadas en esta categoría todavía.</p>
        <?php endif; ?>

        <nav class="arf-pagination" aria-label="Paginación de noticias">
            <?php if ($page > 1): ?>
                <a href="/CaboVision.tv/categoria.php?alias=<?= urlencode($categoryAlias) ?>&page=<?= $page - 1 ?>">&larr; Anterior</a>
            <?php endif; ?>
            <?php if (count($articles) === PER_PAGE): ?>
                <a href="/CaboVision.tv/categoria.php?alias=<?= urlencode($categoryAlias) ?>&page=<?= $page + 1 ?>">Siguiente &rarr;</a>
            <?php endif; ?>
        </nav>
    </div>
