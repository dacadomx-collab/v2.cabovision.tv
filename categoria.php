...
<div class="arf-layout">
    <div class="arf-layout__main">
        ...
        <nav class="arf-pagination" aria-label="Paginación de noticias">
            <?php if ($page > 1): ?>
                <a href="<?= base_path() ?>/categoria.php?alias=<?= urlencode($categoryAlias) ?>&page=<?= $page - 1 ?>">&larr; Anterior</a>
            <?php endif; ?>
            <?php if (count($articles) === PER_PAGE): ?>
                <a href="<?= base_path() ?>/categoria.php?alias=<?= urlencode($categoryAlias) ?>&page=<?= $page + 1 ?>">Siguiente &rarr;</a>
            <?php endif; ?>
        </nav>
    </div>
